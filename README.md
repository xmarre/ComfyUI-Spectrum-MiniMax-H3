...
- `full`: additionally applies a bounded reduced-order correction in the span of the last observed feature change. Alongside the generic Euclidean residual projection, it computes an audio/video candidate weighted by the corresponding native H3 `FinalLayer` output-head Gram diagonal. Completed actual anchors calibrate how much of that model candidate is mixed into the generic control. It does not compute a Jacobian, JVP, or extra denoiser forward.

### Model and LoRA profile

Before a generation, Spectrum lazily builds a compact profile of the effective `ModelPatcher`. It samples at most 4,096 values from each of eight selected matrices in the final H3 block and `FinalLayer`, estimates projection gain, and reads active patch metadata. It also reduces each FP32 `audio_out` / `video_out` matrix over its output axis to the normalized 5,376-channel diagonal of `W^T W`; it never materializes `W^T W`. For ordinary LoRA updates it evaluates the relative Frobenius magnitude of the composed low-rank update directly from the factors. It does not materialize a full `B @ A` update. Large factor sets use a conservative norm bound; unsupported patch forms lower profile confidence and increase uncertainty instead of being reinterpreted as LoRA.

The channel metric is currently the base output-head metric. Existing LoRA identity, perturbation, coverage, and scheduling-risk support remains active, including persistent bypass-hook discovery. Exact patch-conditioned head diagonals are a separate unproven extension; the immediate Feature 3 benchmark therefore targets base H3 with no active LoRA.

Spectrum caches the target `[audio | video]` feature immediately after the last `DiTBlock`. Native H3 then applies `FinalLayer` RMSNorm/AdaLN and the separate FP32 output heads. The static head Gram diagonal is therefore defined on the exact cached hidden-channel axis. It is an output-head metric, not the full timestep-dependent Jacobian through RMSNorm/AdaLN. Earlier-block patches contribute at a lower weight to the separate risk prior. This is a structural prior, not a claim that weight spectra are Spectrum's temporal Chebyshev spectrum.

Profiles are kept in a bounded 16-entry process LRU keyed by ComfyUI's clone lineage UUID, patch-set UUID, H3 architecture signature, and bypass-injection adapter metadata including strength and factor identity. Bypass state is read from the persistent manager or configured hook objects retained by `ModelPatcher.injections`, so reusing a cached loader output does not depend on the loader executing again. Clones with the same effective patch state reuse a profile; `add_patches`, removal/reload, or changed bypass adapters produce a different key. Cached records retain scalars, strings, and the two detached CPU FP32 head diagonals. They retain no model, patch, full weight, or GPU tensor reference.

### Live evidence, replay, and samplers

At actual anchors, Spectrum samples at most 4,096 feature values per modality across the active branches to measure the previous counterfactual forecast/hold ratio, curvature, fit condition, generic residual projection, and `FinalLayer`-weighted projection. The full history retains its configured system-RAM/VRAM policy. A separate bounded sample history stays on the feature device, so evidence reductions do not copy sampled features to CPU; only scalar results return to the controller. The generic and pure model candidates are both evaluated in ordinary feature RMS and head-weighted RMS. Their completed-anchor comparison updates per-stream model trust for the next forecast. A losing model candidate reduces its own influence instead of forcing a model contribution.

The projection EWMAs use a monotonic rational soft limit of 2.0, so extreme finite evidence remains bounded without mapping distinct generic/model observations to the same calibration value. The ±0.25 correction maximum remains a second trust region. Raw generic and model candidates pass through `g / (1 + |g| / 0.25)` before trust mixing. Finite unequal candidates therefore remain unequal after bounding. The applied correction is the convex blend `generic + trust * (model - generic)`, so it cannot leave the same ±0.25 region.
...
Debug mode reports profile cache/build time, retained and temporary workspace estimates, patch coverage, generic/head-weighted projections, raw and smoothly bounded candidates, applied gains, per-stream trust, candidate wins/losses, feature/head-weighted counterfactual ratios, and pre-bound/post-bound/applied deltas. Evidence timing separates fitting, sample/index work, one-time sensitivity materialization, scalar device transfer, reductions, and condition estimation. Causal and offline-replay correction-weight construction timings are separate.
...
- compact model/LoRA profile construction for base, single, stacked, differently weighted, zero-strength, and unknown patches; exact normalized `FinalLayer` head Gram diagonals, clone reuse, patch UUID invalidation, bounded cache lifetime, and no retained model references;
- model-aware risk calibration, bounded adaptive ridge/degree/blend, generic versus head-weighted correction candidates, online model trust, device-local sampled anchor evidence, and risk-only insertion of actual evaluations without relaxing sampler constraints.

The model-aware development suite passes 228 tests against the attached current ComfyUI source in the CPU test environment; 11 native/CUDA cases are skipped where optional runtime dependencies or CUDA are unavailable. GitHub Actions remains the authoritative multi-revision check after this branch is published.

...
| RES4LYF fixed-stage RES | `res_2s`, `res_3s`, `res_5s`, `res_6s` and `*_ode` | Stage 0 stays exact; internal stages use state-conditioned transformer-residual forecasting and are bracketed by exact calls. |
| RES4LYF multistep RES | `res_2m`, `res_3m` and `*_ode` | Full native startup stays exact; later stage-0 forecasts are isolated until 1/2 exact calls respectively flush the forecast from the history slots consumed by the steady-state recurrence. |

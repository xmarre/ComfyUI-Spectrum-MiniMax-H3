...
| RES4LYF fixed-stage RES ODE | `res_2s_ode`, `res_3s_ode`, `res_5s_ode`, `res_6s_ode` | Stage 0 stays exact; internal stages use state-conditioned transformer-residual forecasting and are bracketed by exact calls. |
| RES4LYF multistep RES ODE | `res_2m_ode`, `res_3m_ode` | Full native startup stays exact; later stage-0 forecasts are isolated until 1/2 exact calls respectively flush the forecast from the history slots consumed by the steady-state recurrence. |
| RES4LYF RES SDE | `res_2m`, `res_3m`, `res_2s`, `res_3s`, `res_5s`, `res_6s` | Temporarily native-only: real H3 validation found substantial audio degradation on stochastic `res_2m`; the external SDE noise transition needs a dedicated reviewed policy before forecasts are re-enabled. |
...
RES4LYF support is intentionally narrower than the plugin's full RK surface. Spectrum recognizes the 12 named beta wrappers `res_2m`, `res_3m`, `res_2s`, `res_3s`, `res_5s`, `res_6s` and their `*_ode` variants from reviewed RES4LYF revision `e8437efef69cacf3f08fbd9f90fcc517868c5cb8`, but currently accelerates only the six `*_ode` variants. Real H3 media validation on issue #103 showed significant audio degradation on stochastic `res_2m` while `res_2m_ode` and `res_3s_ode` remained clean, so all six SDE wrappers now fail closed to the untouched native RES4LYF sampler until their external noise transition has a dedicated reviewed Spectrum policy. The generic `rk_beta` sampler, dynamic RK swaps, implicit modes and changed RES4LYF source revisions remain native-only until separately reviewed. The named multistep wrappers' built-in `h >= 1` fallback is part of the audited contract: Spectrum counts the fallback RES/DDIM calls exactly and keeps the entire fallback interval actual rather than forecasting through a runtime tableau change.

Current RES4LYF beta does not call the model for the terminal-zero interval, but before that it may clamp or insert the model's `sigma_min` point. Spectrum mirrors that standard-mode preprocessing before it derives logical-call topology, so the runtime count includes any real `sigma_min` interval that RES4LYF adds. Duplicate-sigma compaction and zero-leading unsampling schedules remain fail-closed. All supported RES4LYF variants use exact-current-state residual forecasting with stage-local histories, so repeated/non-monotonic RK abscissae such as the internal RES stages do not get mixed into one coordinate history. Fixed-stage methods keep stage 0 exact, while multistep methods keep their full startup exact and quarantine each later forecast until it has shifted out of every persistent history slot consumed by the recurrence (one exact outer step for 2M, two for 3M). The final RES4LYF outer interval is always exact; this avoids extrapolating internal-stage coordinates below the runtime's lowest outer-interval coordinate after RES4LYF's terminal `sigma_min` handling. Offline smoothing replay is disabled for this family because replaying altered denoiser values would alter native RK/Adams history and, for the SDE variants, stochastic trajectory state; the default replay setting therefore falls back to one causal Spectrum pass.

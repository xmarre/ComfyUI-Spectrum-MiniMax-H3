...
def _res4lyf_tracking_sigmas(sigmas: Any) -> torch.Tensor | None:
    """Return the sigma intervals on which current RES4LYF beta calls the model."""
...
def min_actual_steps_after_forecast(sampler: Any) -> int:
    name = sampler_name(sampler)
    if name in RES4LYF_MULTISTEP_SAMPLERS:
        # Flush every recurrent RES multistep slot with exact H3 evidence before
        # another forecast may enter persistent native data_prev_ history.
        return RES4LYF_MULTISTEP_PREFIX_STEPS[name]
    if name in RES4LYF_RES_SAMPLERS:
...
def outer_sample_wrapper(
    ...
    if name in RES4LYF_RES_SAMPLERS:
        ...
        schedule_reason = _res4lyf_stage_schedule_reason(sampler, sigmas)
        if preflight_reason is None and schedule_reason is None:
            res4lyf_tracking_sigmas = _res4lyf_tracking_sigmas(sigmas)
            res4lyf_call_topology = _res4lyf_call_topology(sampler, sigmas)
            expected_model_calls = _res4lyf_expected_model_calls(sampler, sigmas)
        if (
    ...
    def execute_run(
        ...
        *,
        ...
        runtime_sigmas = (
            _res4lyf_tracking_sigmas(run_sigmas) if res4lyf_active else run_sigmas
        )
        if res4lyf_active and runtime_sigmas is None:
            raise RuntimeError("RES4LYF active sigma schedule disappeared after preflight")
        ...
        run_id = runtime.start_run(
            ...
            separate_stage_histories=(
                False if active_pece or stochastic_seeds or res4lyf_active else None
            ),
        ...
        if runtime.config.debug:
            LOG.warning(
                ...
                (
                    ...
                    if stochastic_seeds or res4lyf_active

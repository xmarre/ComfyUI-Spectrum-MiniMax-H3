...
from comfyui_spectrum_h3.core_bsa_compat import _hash_git_blob
...
def test_reviewed_res4lyf_checkout_matches_pinned_source_blobs():
    checkout = os.environ.get("SPECTRUM_RES4LYF_PATH")
    if not checkout:
        pytest.skip("reviewed RES4LYF checkout is supplied by the compatibility CI lane")

    root = Path(checkout)
    for contract_name, relative_path in _REVIEWED_RES4LYF_FILES.items():
        ...
        assert blob in RES4LYF_AUDITED_GIT_BLOBS[contract_name], (
            f"reviewed RES4LYF source drifted for {relative_path}: {blob}"
        )
...
def test_outer_wrapper_uses_res4lyf_active_intervals_and_explicit_topology(monkeypatch):
    ...
    # adding one real outer interval and therefore one steady-state 2M model call.
    assert captured["total_steps"] == 7

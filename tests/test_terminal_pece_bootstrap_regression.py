...
def _parsed_terminal_safe_contract() -> compat.ParsedExternalPatchContracts:
    descriptor = compat.ExternalPatchDescriptor(
        ...
        architecture=compat.EXTERNAL_PATCH_ARCHITECTURE,
        ...
        token_tail=0.0,
        cond_only=False,
        scope="visual_reference_attention_modulation",
        terminal_pece_exact_corrector_safe=True,
    )
    return compat.ParsedExternalPatchContracts(
        descriptors=(descriptor,),
        canonical=(descriptor.canonical,),
        fingerprint="terminal-pece-bootstrap-regression",
    )


def _runtime_options(normalized_sigma: float) -> dict:
    return {
        compat.EXTERNAL_PATCH_RUNTIME_KEY: (
            {
                "schema_version": compat.EXTERNAL_PATCH_SCHEMA_VERSION,
                "provider": PROVIDER,
                "instance_id": INSTANCE,
...
def test_terminal_pece_deferral_preserves_one_point_bootstrap_execution_mode():
    ...
    # late hard window and qualifies for the exact same-outer corrector deferral.
    assert compat.observe_external_patch_runtime(runtime, _runtime_options(0.0)) is False

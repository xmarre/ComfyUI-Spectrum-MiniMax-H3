...
For each native FP32 output head `W`, the model profile stores the detached CPU vector `diag(W^T W)`, normalized to mean one. The vector has the same 5,376-channel axis as the cached pre-`FinalLayer` feature. It is a static output-head metric; it does not approximate the timestep-dependent RMSNorm/AdaLN Jacobian. Active patch identity and perturbation remain part of the scheduling prior, while the current channel metric deliberately targets the base output heads.

At a completed actual anchor, the generic control is `<r,d>/<d,d>`. The model candidate is `<r,Sd>/<d,Sd>`, where `S` is the sampled head diagonal, `r` is the counterfactual forecast residual, and `d` is the latest actual feature difference. Both projection streams use monotonic rational soft limits before the ±0.25 correction trust region. The applied gain is a convex generic/model blend controlled by online trust. Trust is updated only when the bounded candidates differ and compares their head-weighted counterfactual errors; scheduling risk and actual-step insertion do not consume this trust.

The full causal history retains its selected storage policy. A separate sample-only history, bounded to 4,096 FP32 values per modality and history entry, stays on the producing feature device. Evidence predictions and Euclidean/head-weighted reductions execute there, then one scalar bundle per stream returns to CPU. Snapshot/restore carries the sample history by tensor reference just like immutable full-history entries; updates replace or append entries and never mutate archived samples.

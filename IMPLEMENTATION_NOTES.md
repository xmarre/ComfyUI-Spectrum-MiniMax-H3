...
At a completed actual anchor, the generic control is `<r,d>/<d,d>`. The diagonal ablation is `<r,Sd>/<d,Sd>`, with `S=diag(W^T W)`. The applied model candidate projects complete sampled hidden rows through the output head and evaluates `<rW^T,dW^T>/<dW^T,dW^T>`, which is algebraically the full `W^T W` metric while retaining cross-channel terms. All three candidates use the same monotonic rational ±0.25 trust region. The applied gain is `generic + trust * (exact - generic)`. Trust is updated only from exact-versus-generic error in exact linear head space; the diagonal candidate remains independently logged and never drives applied trust.

The full causal history retains its selected storage policy. A separate generic evidence history, bounded to 4,096 FP32 values per modality and history entry, stays on the producing feature device. In `full`, deterministic complete-row indices are also retained per stream. Selected hidden rows are projected on-device as each actual anchor arrives, and only the small `[branch, sampled_row, head_output]` representation is archived for exact evidence. Temporal head predictions combine those archived projections directly because the output head is linear. Only one scalar bundle per stream returns to CPU. Snapshot/restore carries both evidence histories and the row-index state by tensor reference; updates replace or append entries and never mutate archived samples.
